...
function(e) {
	...
	}, t.isEnglishVersion = n, t.compareVersion = i
}, function(e, t, n) {
	"use strict";
	...
	var o = n(2),
		...
		s = function(e) {
			...
			return i(t, e), t.prototype.init = function() {
				...
				var e = a.name,
					t = "//www.flash.cn/help/article/article.php?id=114",
					n = "";
	...
	function i(e) {
		return function() {
			...
			return t.prototype.init = function() {
				var e = this;
				this.version = r.$_("ic-version").getElementsByTagName("span")[0], this.download = r.$_("ic-download"), this.aTag = this.download.getElementsByTagName("a")[0], this.size = r.$_("ic-size").getElementsByTagName("span")[0], this.checkbox = new o["default"](r.$_("ic-checkbox")), this.checkbox.on("changed", function() {
					e.update()
				});
				var t = r.$_("ic-button"),
				...
				for (var f = r.$_("ic-popup").getElementsByTagName("a"), d = new l["default"], h = function(e) {
						var t = f[e],
							n = t.href,
							i = t.textContent,
							o = t.innerText;
						r.on(t, "click", function(e) {
							d.showOfModify(i || o, n);
							var t = e || window.event;
							return t.preventDefault ? t.preventDefault() : t.returnValue = !1, !1
						})
					}, v = 0; v < f.length; v++) h(v);
				this.update()
			}, t.prototype.update = function() {
				var e = this;
				this.getPackage(function(t) {
					t && (e.aTag.href = t.downloadURL, e.size.innerHTML = t.size, e.version.innerHTML = t.version), e.checkbox.checked && t ? e.download.className = "" : e.download.className = "disable"
				})
	...
	var i = n(2),
		o = function() {
			...
			return e.prototype.create = function() {
				...
				if (c.length) for (var u = 0; u < c.length; u++) {
					var p = c[u];
	...
	(function(e, o) {
		(function() {
			...
			function s(e, t) {
				var n = -1,
					i = e ? e.length : 0;
				if ("number" == typeof i && i > -1 && i <= w) for (; ++n < i;) t(e[n], n, e);
				else l(e, t)
			}
			function c(e) {
			...
			function v(e) {
				...
				var x = y.navigator || {},
					w = x.userAgent || "";
				e || (e = w);
				var S, O, P = g ? !! x.likeChrome : /\bChrome\b/.test(e) && !/internal|\n/i.test(M.toString()),
					N = "Object",
					...
					z = i(["Adobe AIR", "Arora", "Avant Browser", "Breach", "Camino", "Electron", "Epiphany", "Fennec", "Flock", "Galeon", "GreenBrowser", "iCab", "Iceweasel", "K-Meleon", "Konqueror", "Lunascape", "Maxthon",
					{
				...
				if (X && (X = [X]), D && !U && (U = r([D])), (S = /\bGoogle TV\b/.exec(U)) && (U = S[0]), /\bSimulator\b/i.test(e) && (U = (U ? U + " " : "") + "Simulator"), "Opera Mini" == z && /\bOPiOS\b/.test(e) && $.push("running in Turbo/Uncompressed mode"), "IE" == z && /\blike iPhone OS\b/.test(e) ? (S = v(e.replace(/like iPhone OS/, "")), D = S.manufacturer, U = S.product) : /^iP/.test(U) ? (z || (z = "Safari"), J = "iOS" + ((S = / OS ([\d_]+)/i.exec(e)) ? " " + S[1].replace(/_/g, ".") : "")) : "Konqueror" != z || /buntu/i.test(J) ? D && "Google" != D && (/Chrome/.test(z) && !/\bMobile Safari\b/i.test(e) || /\bVita\b/.test(U)) || /\bAndroid\b/.test(J) && /^Chrome/.test(z) && /\bVersion\//i.test(e) ? (z = "Android Browser", J = /\bAndroid\b/.test(J) ? J : "Android") : "Silk" == z ? (/\bMobi/i.test(e) || (J = "Android", $.unshift("desktop mode")), /Accelerated *= *true/i.test(e) && $.unshift("accelerated")) : "PaleMoon" == z && (S = /\bFirefox\/([\d.]+)\b/.exec(e)) ? $.push("identifying as Firefox " + S[1]) : "Firefox" == z && (S = /\b(Mobile|Tablet|TV)\b/i.exec(e)) ? (J || (J = "Firefox OS"), U || (U = S[1])) : !z || (S = !/\bMinefield\b/i.test(e) && /\b(?:Firefox|Safari)\b/.exec(z)) ? (z && !U && /[\/,]|^[^(]+?\)/.test(e.slice(e.indexOf(S + "/") + 8)) && (z = null), (S = U || D || J) && (U || D || /\b(?:Android|Symbian OS|Tablet OS|webOS)\b/.test(J)) && (z = /[a-z]+(?: Hat)?/i.exec(/\bAndroid\b/.test(J) ? J : S) + " Browser")) : "Electron" == z && (S = (/\bChrome\/([\d.]+)\b/.exec(e) || 0)[1]) && $.push("Chromium " + S) : J = "Kubuntu", G || (G = s(["(?:Cloud9|CriOS|CrMo|Edge|FxiOS|IEMobile|Iron|Opera ?Mini|OPiOS|OPR|Raven|SamsungBrowser|Silk(?!/[\\d.]+$))", "Version", f(z), "(?:Firefox|Minefield|NetFront)"])), (S = "iCab" == X && parseFloat(G) > 3 && "WebKit" || /\bOpera\b/.test(z) && (/\bOPR\b/.test(e) ? "Blink" : "Presto") || /\b(?:Midori|Nook|Safari)\b/i.test(e) && !/^(?:Trident|EdgeHTML)$/.test(X) && "WebKit" || !X && /\bMSIE\b/i.test(e) && ("Mac OS" == J ? "Tasman" : "Trident") || "WebKit" == X && /\bPlayStation\b(?! Vita\b)/i.test(z) && "NetFront") && (X = [S]), "IE" == z && (S = (/; *(?:XBLWP|ZuneWP)(\d+)/i.exec(e) || 0)[1]) ? (z += " Mobile", J = "Windows Phone " + (/\+$/.test(S) ? S : S + ".x"), $.unshift("desktop mode")) : /\bWPDesktop\b/i.test(e) ? (z = "IE Mobile", J = "Windows Phone 8.x", $.unshift("desktop mode"), G || (G = (/\brv:([\d.]+)/.exec(e) || 0)[1])) : "IE" != z && "Trident" == X && (S = /\brv:([\d.]+)/.exec(e)) && (z && $.push("identifying as " + z + (G ? " " + G : "")), z = "IE", G = S[1]), H) {
					if (p(y, "global")) if (I && (S = I.lang.System, j = S.getProperty("os.arch"), J = J || S.getProperty("os.name") + " " + S.getProperty("os.version")), T) {
						try {
							G = y.require("ringo/engine").version.join("."), z = "RingoJS"
						} catch (K) {
							(S = y.system) && S.global.system == y.system && (z = "Narwhal", J || (J = S[0].os || null))
						}
						z || (z = "Rhino")
					} else "object" == typeof y.process && !y.process.browser && (S = y.process) && ("object" == typeof S.versions && ("string" == typeof S.versions.electron ? ($.push("Node " + S.versions.node), z = "Electron", G = S.versions.electron) : "string" == typeof S.versions.nw && ($.push("Chromium " + G, "Node " + S.versions.node), z = "NW.js", G = S.versions.nw)), z || (z = "Node.js", j = S.arch, J = S.platform, G = /[\d.]+/.exec(S.version), G = G ? G[0] : null));
					else u(S = y.runtime) == E ? (z = "Adobe AIR", J = S.flash.system.Capabilities.os) : u(S = y.phantom) == k ? (z = "PhantomJS", G = (S = S.version || null) && S.major + "." + S.minor + "." + S.patch) : "number" == typeof L.documentMode && (S = /\bTrident\/(\d+)/i.exec(e)) ? (G = [G, L.documentMode], (S = +S[1] + 4) != G[1] && ($.push("IE " + G[1] + " mode"), X && (X[1] = ""), G[1] = S), G = "IE" == z ? String(G[1].toFixed(1)) : G[0]) : "number" == typeof L.documentMode && /^(?:Chrome|Firefox)\b/.test(z) && ($.push("masking as " + z + " " + G), z = "IE", G = "11.0", X = ["Trident"], J = "Windows");
					J = J && c(J)
				}
				if (G && (S = /(?:[ab]|dp|pre|[ab]\d+pre)(?:\d+\+?)?$/i.exec(G) || /(?:alpha|beta)(?: ?\d)?/i.exec(e + ";" + (H && x.appMinorVersion)) || /\bMinefield\b/i.test(e) && "a") && (V = /b/i.test(S) ? "beta" : "alpha", G = G.replace(RegExp(S + "\\+?$"), "") + ("beta" == V ? F : B) + (/\d+\+?/.exec(S) || "")), "Fennec" == z || "Firefox" == z && /\b(?:Android|Firefox OS)\b/.test(J)) z = "Firefox Mobile";
				else if ("Maxthon" == z && G) G = G.replace(/\.[\d.]+/, ".x");
				else if (/\bXbox\b/i.test(U))"Xbox 360" == U && (J = null), "Xbox 360" == U && /\bIEMobile\b/.test(e) && $.unshift("mobile mode");
				else if (!/^(?:Chrome|IE|Opera)$/.test(z) && (!z || U || /Browser|Mobi/.test(z)) || "Windows CE" != J && !/Mobi/i.test(e)) if ("IE" == z && H) try {
					null === y.external && $.unshift("platform preview")
				} catch (K) {
					$.unshift("embedded")
				} else(/\bBlackBerry\b/.test(U) || /\bBB10\b/.test(e)) && (S = (RegExp(U.replace(/ +/g, " *") + "/([.\\d]+)", "i").exec(e) || 0)[1] || G) ? (S = [S, /BB10/.test(e)], J = (S[1] ? (U = null, D = "BlackBerry") : "Device Software") + " " + S[0], G = null) : this != l && "Wii" != U && (H && W || /Opera/.test(z) && /\b(?:MSIE|Firefox)\b/i.test(e) || "Firefox" == z && /\bOS X (?:\d+\.){2,}/.test(J) || "IE" == z && (J && !/^Win/.test(J) && G > 5.5 || /\bWindows XP\b/.test(J) && G > 8 || 8 == G && !/\bTrident\b/.test(e))) && !_.test(S = v.call(l, e.replace(_, "") + ";")) && S.name && (S = "ing as " + S.name + ((S = S.version) ? " " + S : ""), _.test(z) ? (/\bIE\b/.test(S) && "Mac OS" == J && (J = null), S = "identify" + S) : (S = "mask" + S, z = R ? c(R.replace(/([a-z])([A-Z])/g, "$1 $2")) : "Opera", /\bIE\b/.test(S) && (J = null), H || (G = null)), X = ["Presto"], $.push(S));
				else z += " Mobile";
				(S = (/\bAppleWebKit\/([\d.]+\+?)/i.exec(e) || 0)[1]) && (S = [parseFloat(S.replace(/\.(\d)$/, ".0$1")), S], "Safari" == z && "+" == S[1].slice(-1) ? (z = "WebKit Nightly", V = "alpha", G = S[1].slice(0, -1)) : G != S[1] && G != (S[2] = (/\bSafari\/([\d.]+\+?)/i.exec(e) || 0)[1]) || (G = null), S[1] = (/\bChrome\/([\d.]+)/i.exec(e) || 0)[1], 537.36 == S[0] && 537.36 == S[2] && parseFloat(S[1]) >= 28 && "WebKit" == X && (X = ["Blink"]), H && (P || S[1]) ? (X && (X[1] = "like Chrome"), S = S[1] || (S = S[0], S < 530 ? 1 : S < 532 ? 2 : S < 532.05 ? 3 : S < 533 ? 4 : S < 534.03 ? 5 : S < 534.07 ? 6 : S < 534.1 ? 7 : S < 534.13 ? 8 : S < 534.16 ? 9 : S < 534.24 ? 10 : S < 534.3 ? 11 : S < 535.01 ? 12 : S < 535.02 ? "13+" : S < 535.07 ? 15 : S < 535.11 ? 16 : S < 535.19 ? 17 : S < 536.05 ? 18 : S < 536.1 ? 19 : S < 537.01 ? 20 : S < 537.11 ? "21+" : S < 537.13 ? 23 : S < 537.18 ? 24 : S < 537.24 ? 25 : S < 537.36 ? 26 : "Blink" != X ? "27" : "28")) : (X && (X[1] = "like Safari"), S = S[0], S = S < 400 ? 1 : S < 500 ? 2 : S < 526 ? 3 : S < 533 ? 4 : S < 534 ? "4+" : S < 535 ? 5 : S < 537 ? 6 : S < 538 ? 7 : S < 601 ? 8 : "8"), X && (X[1] += " " + (S += "number" == typeof S ? ".x" : /[.+]/.test(S) ? "" : "+")), "Safari" == z && (!G || parseInt(G) > 45) && (G = S)), "Opera" == z && (S = /\bzbov|zvav$/.exec(J)) ? (z += " ", $.unshift("desktop mode"), "zvav" == S ? (z += "Mini", G = null) : z += "Mobile", J = J.replace(RegExp(" *" + S + "$"), "")) : "Safari" == z && /\bChrome\b/.exec(X && X[1]) && ($.unshift("desktop mode"), z = "Chrome Mobile", G = null, /\bOS X\b/.test(J) ? (D = "Apple", J = "iOS 4.3+") : J = null), G && 0 == G.indexOf(S = /[\d.]+$/.exec(J)) && e.indexOf("/" + S + "-") > -1 && (J = h(J.replace(S, ""))), X && !/\b(?:Avant|Nook)\b/.test(z) && (/Browser|Lunascape|Maxthon/.test(z) || "Safari" != z && /^iOS/.test(J) && /\bSafari\b/.test(X[1]) || /^(?:Adobe|Arora|Breach|Midori|Opera|Phantom|Rekonq|Rock|Samsung Internet|Sleipnir|Web)/.test(z) && X[1]) && (S = X[X.length - 1]) && $.push(S), $.length && ($ = ["(" + $.join("; ") + ")"]), D && U && U.indexOf(D) < 0 && $.push("on " + D), U && $.push((/^on /.test($[$.length - 1]) ? "" : "on ") + U), J && (S = / ([\d.+]+)$/.exec(J), O = S && "/" == J.charAt(J.length - S[0].length - 1), J = {
					"architecture": 32,
				...
				}), (S = /\b(?:AMD|IA|Win|WOW|x86_|x)64\b/i.exec(j)) && !/\bi686\b/i.test(j) ? (J && (J.architecture = 64, J.family = J.family.replace(RegExp(" *" + S), "")), z && (/\bWOW64\b/i.test(e) || H && /\w(?:86|32)$/.test(x.cpuClass || x.platform) && !/\bWin64; x64\b/i.test(e)) && $.unshift("32-bit")) : J && /^OS X/.test(J.family) && "Chrome" == z && parseFloat(G) >= 39 && (J.architecture = 64), e || (e = null);
				var q = {};
				return q.description = e, q.layout = X && X[0], q.manufacturer = D, q.name = z, q.prerelease = V, q.product = U, q.ua = e, q.version = z && G, q.os = J || {
					"architecture": null,
					"family": null,
					"version": null,
					"toString": function() {
				...
				}, q.parse = v, q.toString = b, q.version && $.unshift(G), q.name && $.unshift(z), J && z && (J != String(J).split(" ")[0] || J != z.split(" ")[0] && !U) && $.push(U ? "(" + J + ")" : "on " + J), $.length && (q.description = $.join(" ")), q
			}
			var b = {
				"function": !0,
				"object": !0
			...
			return i(t, e), t.prototype.getID = function() {
				...
				l.on("changed", function() {
					...
					e && n.protocolCheckBox.checked && e(function(e) {
						n.start.className = "", n.start.getElementsByTagName("a")[0].href = e.downloadURL, n.total.innerHTML = e.size
					})
				});
				...
				return p.on("changed", function(e) {
					...
					if (i) {
						...
						n.start.className = "disable", n.start.getElementsByTagName("a")[0].removeAttribute("href"), n.total.innerHTML = "0.00MB";
				...
				e.prototype.hide.call(this), this.start.className = "disable", this.start.getElementsByTagName("a")[0].removeAttribute("href"), this.total.innerHTML = "0.00MB", this.sysCombo.clean(), this.verCombo.clean(), r.$_("ov-condition-provision").innerHTML = ""
			}, t.strategies = p, t
		}(o["default"]);
	t["default"] = f
}, function(e, t) {
	...
	function() {
		...
		function str(e, t) {
			...
			case "object":
				...
				if (rep && "object" == typeof rep) for (r = rep.length, n = 0; n < r; n += 1)"string" == typeof rep[n] && (i = rep[n], o = str(i, c), o && a.push(quote(i) + (gap ? ": " : ":") + o));
				else for (i in c) Object.prototype.hasOwnProperty.call(c, i) && (o = str(i, c), o && a.push(quote(i) + (gap ? ": " : ":") + o));
				return o = 0 === a.length ? "{}" : gap ? "{\n" + gap + a.join(",\n" + gap) + "\n" + s + "}" : "{" + a.join(",") + "}", gap = s, o
			}
		}
